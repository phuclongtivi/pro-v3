...
export default function AIJobs(){
 ...
 useEffect(()=>{load()},[])
 return <main className="page"><section className="hero"><h1>AI Jobs</h1><p>AI làm việc có trạng thái • approval • result</p></section><section className="panel" style={{marginTop:12}}><div className="field"><label>Job</label><select value={type} onChange={e=>setType(e.target.value)}>{types.map(x=><option key={x}>{x}</option>)}</select></div><button data-action-id="pro.app.ai.jobs.page.button.001" className="action" onClick={create}>Tạo AI Job</button></section><section className="grid" style={{marginTop:12}}>{jobs.map(j=><div className="card" key={j.id}><h3>{j.job_type}</h3><p>{j.status}</p><div className="layerbar">{j.status==="pending_approval"&&<button data-action-id="pro.app.ai.jobs.page.button.002" onClick={()=>act(j.id,"approve")}>Approve</button>}{j.status==="queued"&&<button data-action-id="pro.app.ai.jobs.page.button.003" onClick={()=>act(j.id,"execute")}>Execute</button>}</div></div>)}</section><BrandFooter/></main>
}