...
export default function EventSpace(){
 ...
 if(!joined)return <main className="page"><section className="eventGate"><span className="eyebrow">EVENT GATE</span><h1>{event.title}</h1><p>Join first, install later.</p><div className="guestJoin"><input value={name} onChange={e=>setName(e.target.value)} placeholder="Tên hiển thị"/><button data-action-id="pro.app.event.token.page.button.001" className="action" onClick={join}>Tiếp tục vào sự kiện</button><span>Không bắt buộc cài app hoặc đăng nhập. Có thể liên kết tài khoản sau.</span></div>{qr&&<img src={qr} alt="Event QR" style={{width:220,borderRadius:16}}/>}</section></main>;
 ...
 return <main className="page eventSpace">
  ...
  <div className="serviceDock">{services.map(x=><button data-action-id={`pro.event.service.${x.toLowerCase().replace(/[^a-z0-9]+/g,"-")}`} key={x} className={service===x?"active":""} onClick={()=>setService(x)}>{x}</button>)}</div>
