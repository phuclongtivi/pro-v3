...
export default function Page(){
 ...
 return <main className="page nav3Page"><div className="layerbar corebar nav3Corebar">{groups.map(x=><button data-action-id="pro.app.me.page.button.001" key={x.id} className={id===x.id?"active":""} onClick={()=>choose(x)}>{label(x.label,lang)}</button>)}</div><Nav3Navigator section={group.id} items={group.children} activeId={child.id} onSelect={setChildId} lang={lang}/><BrandFooter/></main>}
