...
export default function Page(){
 ...
 return <main className="page nav3Page"><div className="layerbar corebar nav3Corebar">{groups.map(x=><button data-action-id={`pro.store.group.${x.id}`} key={x.id} className={id===x.id?"active":""} onClick={()=>choose(x)}>{label(x.label,lang)}</button>)}</div><BrandZoneDivider/><Nav3Navigator section={group.id} items={group.children} activeId={child.id} onSelect={setChildId} lang={lang}/><BrandFooter/></main>}
