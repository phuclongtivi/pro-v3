...
export function LanguageProvider({children}:{children:React.ReactNode}){
 ...
 return <C.Provider value={{lang,setLang,t}}>{children}{ready&&showGate&&<div className="languageGate"><div className="languageCard"><span className="eyebrow">LONG APP</span><h1>{t("chooseLanguage")}</h1><div className="languageChoices">{(Object.keys(languages) as Lang[]).map(v=><button data-action-id={`pro.language.${v}.select`} key={v} onClick={()=>firstChoose(v)} className={lang===v?"active":""}>{languages[v]}</button>)}</div><p>{t("changeLater")}</p></div></div>}{welcome&&<WelcomeExperience lang={lang} onDone={finishWelcome}/>}</C.Provider>
}
