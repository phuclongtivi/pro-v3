...
export default function ProductCreateForm({lang,onBack,onDone,mode="new"}:Props){
 ...
 return <section className="navWorkspace specializedPanel">
  <div className="workspaceCrumbs"><button data-action-id="pro.components.productcreateform.button.001" type="button" className="backKey" onClick={onBack}>← Back</button><span className="crumbKey selected">{vi?"Tạo sản phẩm":"Create Product"}</span><span className="crumbKey selected tree5Crumb">{mode==="template"?(vi?"Từ mẫu":"From Template"):(vi?"Mới":"New")}</span></div>
