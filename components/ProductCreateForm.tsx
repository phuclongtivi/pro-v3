...
export default function ProductCreateForm({lang,onBack,onDone,mode="new"}:Props){
 ...
 return <section className="navWorkspace specializedPanel">
  <div className="workspaceCrumbs"><button data-action-id="pro.components.productcreateform.button.001" type="button" className="backKey" onClick={onBack}>← Back</button><button data-action-id="pro.components.productcreateform.button.002" type="button" className="crumbKey selected">{vi?"Tạo sản phẩm":"Create Product"}</button><button data-action-id="pro.components.productcreateform.button.003" type="button" className="crumbKey selected tree5Crumb">{mode==="template"?(vi?"Từ mẫu":"From Template"):(vi?"Mới":"New")}</button></div>
  ...
  <div className="endCommitRow"><button data-action-id="pro.components.productcreateform.button.004" type="button" className="secondaryEnd" onClick={()=>setDraft(true)}>{draft?"✓ ":""}{vi?"Lưu nháp":"Save draft"}</button><button data-action-id="pro.components.productcreateform.button.005" type="button" className="endCommit" disabled={!valid} onClick={onDone}>{vi?"Lưu & Đăng sản phẩm":"Save & Publish Product"}</button></div>
 </section>
}
