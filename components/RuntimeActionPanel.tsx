...
export default function RuntimeActionPanel({lang,title,onCancel,onComplete}:{lang:Lang;title:string;onCancel:()=>void;onComplete:()=>void}){
 ...
 return <div className="functionalPanel runtimeActionPanel"><div className="functionalHead"><div><span>RUNTIME CONTENT</span><h2>{title}</h2></div><b className={`statePill ${state==="tested"?"live":"ready"}`}>{state==="tested"?(vi?"ĐÃ KIỂM TRA":"TESTED"):(vi?"SẴN SÀNG":"READY")}</b></div>
  ...
  <div className="runtimeStates"><button data-action-id="pro.components.runtimeactionpanel.button.001" type="button" onClick={()=>setState("tested")}>{vi?"Kiểm tra runtime":"Check runtime"}</button><span>{state==="tested"?(vi?"✓ Runtime đã phản hồi trạng thái hợp lệ":"✓ Runtime returned a valid state"):(vi?"○ Chưa kiểm tra":"○ Not checked")}</span></div>
  <div className="endCommitRow"><button data-action-id="pro.components.runtimeactionpanel.button.002" type="button" className="secondaryEnd" onClick={onCancel}>{vi?"Chọn lại":"Choose again"}</button><button data-action-id="pro.components.runtimeactionpanel.button.003" type="button" className="endCommit" disabled={state!=="tested"} onClick={onComplete}>{vi?"Áp dụng":"Apply"}</button></div></div>
}
