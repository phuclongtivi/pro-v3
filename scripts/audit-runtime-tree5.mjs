...
const failures=[];
for(const x of ["gift-manage","gift-create","room-create","room-name","graphics-overlay","custom-api","customer-orders","profile"]) if(!sem.includes(`intent===\"${x}\"`)) failures.push(`missing R4B runtime panel ${x}`);
...
if(!sem.includes('event-notice-viewer')||!sem.includes('event-notice-editor')) failures.push('role-aware event notice runtime panels missing');
