...
for(const x of ['qr-create','external-app','display-output','ai-bill','ai-ticket','ai-sales','device','audio-control','media-render','inventory','orders','gift-manage','gift-create','room-create','room-name','graphics-overlay','custom-api','customer-orders','profile']) if(!sem.includes(`"${x}"`)) failures.push(`missing semantic intent ${x}`);
if(!sem.includes('event-notice-viewer')||!sem.includes('event-notice-editor')) failures.push('missing role-aware event notice intents');
